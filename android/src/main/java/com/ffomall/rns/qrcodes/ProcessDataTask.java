...
public class ProcessDataTask extends AsyncTask<Void, Void, ScanResult> {
    ...
    protected ScanResult doInBackground(Void... params) {
        ...
        if (mPicturePath != null) {
            return qrCodeView.processBitmapData(WstQRCodeUtils.getDecodeAbleBitmap(mPicturePath));
            ...
            if (WstQRCodeUtils.isDebug()) {
                WstQRCodeUtils.d("两次任务执行的时间间隔：" + (System.currentTimeMillis() - sLastStartTime));
            ...
            if (WstQRCodeUtils.isDebug()) {
                ...
                if (scanResult != null && !TextUtils.isEmpty(scanResult.result)) {
                    WstQRCodeUtils.d("识别成功时间为：" + time);
                } else {
                    WstQRCodeUtils.e("识别失败时间为：" + time);
                }
            }
