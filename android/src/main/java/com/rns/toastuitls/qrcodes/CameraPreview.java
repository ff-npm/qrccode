...
public class CameraPreview extends SurfaceView implements SurfaceHolder.Callback {
    ...
    void onScanBoxRectChanged(Rect scanRect) {
        ...
        WstQRCodeUtils.printRect("转换前", scanRect);

        if (WstQRCodeUtils.isPortrait(getContext())) {
            int temp = centerX;
            centerX = centerY;
            centerY = temp;
        ...
        WstQRCodeUtils.printRect("转换后", scanRect);

        WstQRCodeUtils.d("扫码框发生变化触发对焦测光");
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (event.getPointerCount() == 1 && (event.getAction() & MotionEvent.ACTION_MASK) == MotionEvent.ACTION_UP) {
            ...
            int focusSize = WstQRCodeUtils.dp2px(getContext(), 120);
        ...
        if (event.getPointerCount() == 2) {
            switch (event.getAction() & MotionEvent.ACTION_MASK) {
                case MotionEvent.ACTION_POINTER_DOWN:
                    mOldDist = WstQRCodeUtils.calculateFingerSpacing(event);
                    break;
                case MotionEvent.ACTION_MOVE:
                    float newDist = WstQRCodeUtils.calculateFingerSpacing(event);
                    if (newDist > mOldDist) {
    ...
    private static void handleZoom(boolean isZoomIn, Camera camera) {
        ...
        if (params.isZoomSupported()) {
            ...
            if (isZoomIn && zoom < params.getMaxZoom()) {
                ...
                WstQRCodeUtils.d("既不放大也不缩小");
            }
            params.setZoom(zoom);
            camera.setParameters(params);
        } else {
            WstQRCodeUtils.d("不支持缩放");
        }
    }
    ...
    private void handleFocusMetering(float originFocusCenterX, float originFocusCenterY,
        ...
        try {
            ...
            if (focusMeteringParameters.getMaxNumFocusAreas() > 0) {
                WstQRCodeUtils.d("支持设置对焦区域");
                isNeedUpdate = true;
                Rect focusRect = WstQRCodeUtils.calculateFocusMeteringArea(1f,
                        originFocusCenterX, originFocusCenterY,
                        originFocusWidth, originFocusHeight,
                        size.width, size.height);
                WstQRCodeUtils.printRect("对焦区域", focusRect);
                ...
                WstQRCodeUtils.d("不支持设置对焦区域");
            }

            if (focusMeteringParameters.getMaxNumMeteringAreas() > 0) {
                WstQRCodeUtils.d("支持设置测光区域");
                isNeedUpdate = true;
                Rect meteringRect = WstQRCodeUtils.calculateFocusMeteringArea(1.5f,
                        originFocusCenterX, originFocusCenterY,
                        originFocusWidth, originFocusHeight,
                        size.width, size.height);
                WstQRCodeUtils.printRect("测光区域", meteringRect);
                focusMeteringParameters.setMeteringAreas(Collections.singletonList(new Camera.Area(meteringRect, 1000)));
            } else {
                WstQRCodeUtils.d("不支持设置测光区域");
            }
            ...
            if (isNeedUpdate) {
                ...
                mCamera.autoFocus(new Camera.AutoFocusCallback() {
                    public void onAutoFocus(boolean success, Camera camera) {
                        if (success) {
                            WstQRCodeUtils.d("对焦测光成功");
                        } else {
                            WstQRCodeUtils.e("对焦测光失败");
                        }
                        startContinuousAutoFocus();
                    }
                });
            ...
            e.printStackTrace();
            WstQRCodeUtils.e("对焦测光失败：" + e.getMessage());
            startContinuousAutoFocus();
        }
    }
