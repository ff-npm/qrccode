...
public abstract class QRCodeView extends RelativeLayout implements Camera.PreviewCallback {
    ...
    public void onPreviewFrame(final byte[] data, final Camera camera) {
        if (WstQRCodeUtils.isDebug()) {
            WstQRCodeUtils.d("两次 onPreviewFrame 时间间隔：" + (System.currentTimeMillis() - mLastPreviewFrameTime));
        ...
        mProcessDataTask = new ProcessDataTask(camera, data, this, WstQRCodeUtils.isPortrait(getContext())).perform();
    }
    ...
    private void handleAmbientBrightness(byte[] data, Camera camera) {
        ...
        if (Math.abs(data.length - pixelCount * 1.5f) < 0.00001f) {
            ...
            WstQRCodeUtils.d("摄像头环境亮度为：" + cameraLight);
            if (mDelegate != null) {
    ...
    protected boolean transformToViewCoordinates(final PointF[] pointArr, final Rect scanBoxAreaRect, final boolean isNeedAutoZoom, final String result) {
        ...
        try {
            ...
            int statusBarHeight = WstQRCodeUtils.getStatusBarHeight(getContext());
